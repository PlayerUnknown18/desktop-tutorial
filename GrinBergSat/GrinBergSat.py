...
class DoplerCorrection:
    def __init__(self, jinfo_object, device):
        ...
        if device == "hdsdr":
            ...
            self.port_comm = port_communication.DoplerPortCommunication(
                self.port, self.satellite_freq)
        ...
        if jinfo_object.tle_type == "file":
            self.doppler_satellite = self.load_tle_file(r"C:\Users\GroundStation\Documents\GitHub\desktop-tutorial\GrinBergSat\tle.txt")
        else:
            self.doppler_satellite = load.tle(
                "https://celestrak.com/NORAD/elements/active.txt", reload=False)[self.satellite_name]

        self.dopler_station = Topos(
            jinfo_object.station_lat, jinfo_object.station_lon)

    def load_tle_file(self,filename):
        return load.tle_file(filename)[0]
    ...
    def calculate_dopler(self, freq, offset, mode):
        ...
        if mode == "uplink":
            dopler_corr = (freq * (self.__speed_of_light -
                                   change) / self.__speed_of_light) - offset
        elif mode == "downlink":
            dopler_corr = (freq * (self.__speed_of_light +
                                   change) / self.__speed_of_light) - offset
        else:
    ...
    def update_dopler_hdsdr(self):
        while True:
            time.sleep(0.3)
            dopler_freq = self.calculate_dopler(
                self.satellite_freq, self.__offset, "downlink")
            print(dopler_freq)
    ...
    def update_dopler_rfcb(self, sock_io):
        ...
        while True:
            ...
            get_freq = self.calculate_dopler(
                self.rfcb_freq, 0, "uplink") * 10 ** 6  # 145.970 -> 145970
            time.sleep(2)
...
class UpdateSatelliteCords:
    def __init__(self, jinfo_object):
        ...
        self.satellite_name = get_sat_name(int(self.config_data.norad_id))
        if jinfo_object.tle_type == "file":
            self.satellite = self.load_tle_file(r"C:\Users\GroundStation\Documents\GitHub\desktop-tutorial\GrinBergSat\tle.txt")
        else:
            self.satellite = load.tle(
                "https://celestrak.com/NORAD/elements/active.txt", reload=False)[self.satellite_name]
        ...
        self.station = Topos(jinfo_object.station_lat,
                             jinfo_object.station_lon)
    ...
    def update_satellite_cords(self):
        ...
        while True:
            ...
            orbital_object = pyorbital.orbital.Orbital(
                self.satellite_name, "active.txt")
            self.azimuth, self.elevation = orbital_object.get_observer_look(
                utc_time_now, self.station_lon, self.station_lat, self.station_alt)
            ...
            if int(last_azimuth) != int(self.azimuth) or int(last_elevation) != int(self.elevation):
                ...
                if self.check_if_real_cords(self.azimuth, self.elevation):
                    self.azimuth, self.elevation = self.port_comm.convert_to_format(
                        self.azimuth, self.elevation)
                    self.port_comm.write_antena_coordinates_to_port(
                        self.azimuth, self.elevation)
...
def update_modulation(sock_io):
    FSK_CODE = [9, 19, 0, 0, 0, 9, 70, 83, 75,
                45, 71, 51, 82, 85, 72, 128, 37, 0, 0]
...
def connect_to_sock():
    ...
    while True:
        try:
            ...
            print(
                "cant connect to rfcb,please run the rf checkout box\rtry again in 5 seconds")
            time.sleep(5)
    return socket_conn
...
def get_sat_name(norad):
    satellite = requests.get(
        f"https://celestrak.com/satcat/tle.php?CATNR={norad}").text
